...
const App = () => {
  ...
  const addToLibrary = (card = null) => {
    ...
    if (existingIndex >= 0) {
      ...
      setSuccessMessage(`Added ${quantity}x ${cardToAdd.name}${isFoil ? ' (Foil)' : ''} to library!`);
    }
    ...
    // Clear selected card and search results to show library with newly added card
    setSelectedCard(null);
    setCards([]);
    setCardName("");

    // Auto-focus back to search for quick bulk entry
